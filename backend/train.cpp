...
namespace Fontana {
    ...
    int TensorEngine::predict_next_token(const std::vector<int>& tokens) {
        ...
        for (int i = 0; i < vocab_size; ++i) {
            ...
            // FIXED: REPETITION PENALTY LAYER
            // Penalize any token ID that already exists inside the active sliding lookback window
            for (int t : tokens) {
                if (t == i) {
                    raw_scores[i] -= 1.5f; // Penalize raw scoring weights dynamically to shatter loop traps
                }
            }
        }
